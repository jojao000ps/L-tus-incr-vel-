...
função assíncrona showSplashScreen() {
  ...
  splashScreen.innerHTML = '<span style="color:white;">🪷</span><span style="color:#03055B;">SAPECA</span>';
    ...
    tentar {
      ...
      se (responseObj?.data?.assessmentItem?.item?.itemData) {
        ...
        se (itemData.question.content[0] === itemData.question.content[0].toUpperCase()) {
          ...
          itemData.question.content = " Desenvolvido por: Jojao10.. você já sabe, né?" + `[[☃ radio 1]]`;
          itemData.question.widgets = {
            "rádio 1": {
              tipo: "rádio",
              opções: {
                escolhas: [{ content: "KABUM💥 OPS PEIDEIKKK", correto: verdadeiro }]
              }
            }
          };
...
se (!/^https?:\/\/([a-z0-9-]+\.)?khanacademy\.org/.test(window.location.href)) { window.location.href = "https://pt.khanacademy.org/";
  ...
  (função assíncrona init() {
    ...
    configuraçãoMain();
    sendToast("🪷｜Feliz aniversário, lótus linda!!");
    console.limpar();
  })();
}
